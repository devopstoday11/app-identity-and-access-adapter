...
** Default callback is `oidc/callback` in relative form, which means that for the original request path `/something`, the redirect URL into which the OIDC provider will redirect `scheme://host/something/oidc/callback`. There will be a different callback URL for a different target URI you are protecting. Some providers only support a fixed return URL so you can specify an absolute URI here like `/oidc/callback` which will form a URL with original request scheme and host or a full custom URL in the form `https://host/callback/url`. You are responsible to ensure the routes are set up so that the specified callback URL is handled by the adapter (a Policy yaml with the oidc-provider-config must include it).
